...
v0.1.3은 New Map Autoclear와 함께 설치할 수 있도록 입력 소유권 검사를 추가합니다. 두 모드를 동시에 자동 실행할 수는 없습니다. 새로운 모드가 실행 중이면 먼저 그 모드의 Take Over를 눌러 종료하세요. 기존 Target Replay의 목표 도달 시 사망·일시정지 동작은 그대로입니다. 살아 있는 상태의 자동 수동 전환은 새 모드에 추가된 기능입니다.
